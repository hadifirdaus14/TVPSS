        ...
        http
                ...
                .formLogin(login -> login
                        .successHandler((request, response, authentication) -> {
                            ...
                            response.sendRedirect("http://localhost:8080/TVPSS/");
                            
                            User user = new User();
                            
                            switch (role) {
                          case "CREW":
                            	request.getSession().setAttribute("name", crewDao.getCrewByUserId(user.getId()).getName());
                                response.sendRedirect("http://localhost:8080/TVPSS-1/ManageResource");
                                break;
                            case "PIC":
                            	request.getSession().setAttribute("name", picDao.getPICByUserId(user.getId()).getName());
                                response.sendRedirect("http://localhost:8080/TVPSS-1/manageSchoolInfo");
                                break;
                            case "ADMIN":
                                response.sendRedirect("http://localhost:8080/TVPSS-1/manageUser");
                                break;
                            default:
